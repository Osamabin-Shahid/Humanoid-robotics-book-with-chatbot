...
def test_basic_retrieval():
    ...
    try:
        ...
        print(f"SUCCESS: Retrieved {len(results)} results for query: '{test_query}'")
        return True
    except Exception as e:
        print(f"WARNING: Retrieval failed (may be expected if no data in Qdrant): {e}")
        return False
...
def test_retrieval_with_validation():
    ...
    try:
        ...
        if results:
            ...
            print(f"SUCCESS: Validated {len(results)} results with {len(validation_reports)} validation reports")
            return True
        else:
            print("WARNING: No results to validate (expected if no data in Qdrant)")
            return True
    except Exception as e:
        print(f"WARNING: Validation test failed (may be expected if no data in Qdrant): {e}")
        return False
...
def test_pipeline_validation():
    ...
    try:
        ...
        print(f"SUCCESS: Pipeline validation completed with success rate: {report.get('success_rate', 0)}")
        return True
    except Exception as e:
        print(f"WARNING: Pipeline validation failed (may be expected if no data in Qdrant): {e}")
        return False
...
def test_api_endpoints():
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            print(f"WARNING: Health endpoint returned status: {health_response.status_code}")
            return False
        ...
        if val_health_response.status_code == 200:
            print("SUCCESS: Validation health endpoint is accessible")
        else:
            print(f"WARNING: Validation health endpoint returned status: {val_health_response.status_code}")
            return False
        ...
        if retrieval_response.status_code in [200, 400, 500]:  # 400 is expected for bad queries, 500 for missing data
            print("SUCCESS: Retrieval endpoint is accessible")
        else:
            print(f"WARNING: Retrieval endpoint returned unexpected status: {retrieval_response.status_code}")
            return False

        return True
    except requests.exceptions.ConnectionError:
        print("WARNING: API endpoints not accessible (server may not be running)")
        return False
    except Exception as e:
        print(f"WARNING: Error testing API endpoints: {e}")
        return False
...
def test_error_handling():
    ...
    try:
        ...
        print("WARNING: Empty query should have raised ValueError")
        return False
    except ValueError:
        print("SUCCESS: Empty query validation works correctly")
    except Exception as e:
        print(f"WARNING: Unexpected error for empty query: {e}")
        return False

    try:
        # This should raise a ValueError
        retrieval_service.retrieve("valid query", limit=0, min_score=0.0)
        print("WARNING: Zero limit should have raised ValueError")
        return False
    except ValueError:
        print("SUCCESS: Zero limit validation works correctly")
    except Exception as e:
        print(f"WARNING: Unexpected error for zero limit: {e}")
        return False

    try:
        # This should raise a ValueError
        retrieval_service.retrieve("valid query", limit=5, min_score=1.5)
        print("WARNING: Invalid min_score should have raised ValueError")
        return False
    except ValueError:
        print("SUCCESS: Invalid min_score validation works correctly")
    except Exception as e:
        print(f"WARNING: Unexpected error for invalid min_score: {e}")
        return False
...
def run_all_tests():
    ...
    for test_name, test_func in tests:
        ...
        try:
            ...
            if result:
                ...
                print(f"SKIPPED: {test_name} test failed or skipped")
        except Exception as e:
    ...
    if passed == total:
        ...
        print(f"WARNING: {total - passed} tests failed or were skipped")
        return False
